...
- **Spokes require `CAP_NET_ADMIN` + iptables to start enforcing.** If the chain cannot be created, the entrypoint exits with:
  ...
  This FATAL exits with a **distinct exit code, 77** (sysexits.h `EX_NOPERM`), rather than the generic `1` every other startup failure in this script uses — specifically when the bounding set lacks `CAP_NET_ADMIN` (netfilter chain manipulation requires it, so that absence is by itself sufficient to explain the failure). A supervisor or `lastState.terminated.exitCode` can key off 77 to mean "grant the capability" without parsing the log; any other cause of the same FATAL (missing `iptables` binary, netfilter lock contention) still exits `1`. See [net-admin-requirement.md](net-admin-requirement.md#when-the-container-refuses-to-start-exit-77).
