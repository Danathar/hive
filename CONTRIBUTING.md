...
| --- | --- |
| `v5` (default) | Bug fixes, security fixes, dependency updates, docs, operability improvements, and structural or protocol-level changes (the latter gated by public `[v5 RFC]` issues per [ROADMAP.md](ROADMAP.md#v5--next-generation)). `v5` is the stable line: it publishes `candidate`/`latest` and is promoted to `stable` after soak. |
| `v4` | **Feature-frozen maintenance line** (in effect since 2026-09-21, v4 SHA `c354a5008`; policy [#6346](https://github.com/hivecommons/hive/issues/6346), [ROADMAP.md](ROADMAP.md#v4--stable-line)): security fixes and critical fixes only. The required `freeze-gate` check (repository variable `V4_FEATURE_FREEZE`) rejects any v4 PR not carrying `security`, `agent/security`, `priority/critical-urgent`, or `v4-freeze-exempt` and escalates it `needs-human` — retarget anything else at `v5`. v4→v5 movement is cherry-pick-only. Runbook: [src/docs/v4-freeze-runbook.md](src/docs/v4-freeze-runbook.md). |
...
- **This binds manual closes too.** The hive now routes App-bot `gh issue close` requests and in-process close calls through the same reporter-confirmation gate as PR closing keywords. Closing a human-filed issue by hand from the issue page is subject to the same confirm-or-7-days rule; if an agent needs a legitimate duplicate/not-a-bug/reporter-requested close before the marker is present, it must use the explicit override path and record the reason in the issue timeline. Issues [#6962](https://github.com/hivecommons/hive/issues/6962) and [#6765](https://github.com/hivecommons/hive/issues/6765) were closed manually while the reporter considered them unresolved, and the same symptoms came back as [#7092](https://github.com/hivecommons/hive/issues/7092) — the exact re-file loop this section exists to prevent ([#7125](https://github.com/hivecommons/hive/issues/7125)).
- **State deployment reach, not just merge status.** For a reporter on a hosted or auto-updating hive, "fixed" means *running where they are looking*, not merged to a branch. A status comment on an adopter-filed issue should say which channel or deployment the fix has reached — or that it has not shipped yet. On [#7092](https://github.com/hivecommons/hive/issues/7092), fixes were reported as done while the reporter's deployment predated both merge commits, so the reporter verified against a build that could not contain them.
- **Enforced at PR-request and direct-close time.** The hive's shared detector treats an issue as reporter-gated when it looks like a human-filed bug (a bug-family label — `bug`, `kind/bug`, `type/bug`, `type:bug`, `adoption-blocker` — with no `— hive:` attribution trailer and a non-`Bot` author). The PR-request watcher (`pkg/github.validatePRRequestClaims`) rewrites `Closes #N` to `Refs #N`, so a merge cannot auto-close it under the App bot; the issue-close path refuses a direct close, posts a confirmation request, and leaves the issue open. The reporter (or a maintainer with write access) can opt back in by adding `hive: reporter-confirmed` to the issue body or applying that label; the gate then lets `Closes #N` and direct closes through untouched. Tracked in [#6781](https://github.com/hivecommons/hive/issues/6781).
