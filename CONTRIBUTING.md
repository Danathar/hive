...
| --- | --- |
| `v4` (default) | Bug fixes, security fixes, dependency updates, docs, and operability improvements on the supported stable line. Note the accepted [feature-freeze policy](ROADMAP.md#v4--stable-line) ([#6346](https://github.com/hivecommons/hive/issues/6346)): the freeze is **in effect since 2026-09-21** (v4 SHA `c354a5008`); `v4` accepts security and critical fixes only. The freeze is enforced by the required `freeze-gate` check (repository variable `V4_FEATURE_FREEZE` is set): a post-freeze v4 PR must carry `security`, `agent/security`, `priority/critical-urgent`, or `v4-freeze-exempt`, or it fails the gate and is escalated `needs-human` — retarget anything else at `v5`. Execution steps: [src/docs/v4-freeze-runbook.md](src/docs/v4-freeze-runbook.md). |
