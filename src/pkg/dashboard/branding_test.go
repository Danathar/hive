...
func TestInjectBrandingPlacesLinkBeforeHeadClose(t *testing.T) {
	got := webstatic.InjectBranding([]byte("<html><head><title>x</title></head><body>b</body></html>"))
	if !bytes.Contains(got, []byte(webstatic.BrandingLinkTag)) {
		t.Fatalf("override link not injected: %s", got)
	}
	// It must be LAST in head, or the embedded document's own styles win.
	if bytes.Index(got, []byte(webstatic.BrandingLinkTag)) > bytes.Index(got, []byte("</head>")) {
...
func TestInjectBrandingNoHeadIsInert(t *testing.T) {
	...
	if got := webstatic.InjectBranding(in); !bytes.Equal(got, in) {
...
// Testing webstatic.InjectBranding() alone passes even if the server wiring
// never calls it, so this goes through the same composition Start() uses —
// InjectBranding into the real document builder — and the real handler.
func TestServedIndexCarriesBrandingLink(t *testing.T) {
	doc := webstatic.NewIndexDocument(webstatic.InjectBranding(applyBranding(
		[]byte("<html><head><title>hive</title></head><body></body></html>"), Branding{})))
	...
	if !bytes.Contains(w.Body.Bytes(), []byte(webstatic.BrandingLinkTag)) {
...
func TestServedIndexCarriesBrandingStrings(t *testing.T) {
	doc := webstatic.NewIndexDocument(webstatic.InjectBranding(
		applyBranding([]byte(sampleIndex), Branding{ProductName: "SCHOOL"})))
	req := httptest.NewRequest(http.MethodGet, "/", nil)
