...
func TestSSOHandoff_ValidTokenLandsOnRootWithSession(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			tok := spoke.MintSSOToken(spoke.SSOSigningSeedFromMaster(testHubSecret), "clubanderson", config.RoleOwner, testHiveID, time.Now())
			if tok == "" {
...
func TestSSOHandoff_FailuresTerminateWithoutRedirecting(t *testing.T) {
	validTok := func() string {
		return spoke.MintSSOToken(spoke.SSOSigningSeedFromMaster(testHubSecret), "clubanderson", config.RoleOwner, testHiveID, time.Now())
	}
	...
	tests := []struct {
		...
		},
		{
			...
			query: func() string {
				...
				return "token=" + spoke.MintSSOToken(spoke.SSOSigningSeedFromMaster("a-different-master"), "clubanderson", config.RoleOwner, testHiveID, time.Now())
			},
			wantStatus: http.StatusUnauthorized,
			wantCode:   ssoErrBadToken,
		},
		{
			...
			hubSecret:  testHubSecret,
			query: func() string {
				return "token=" + spoke.MintSSOToken(spoke.SSOSigningSeedFromMaster(testHubSecret), "clubanderson", config.RoleOwner, "some-other-hive", time.Now())
			},
			wantStatus: http.StatusUnauthorized,
			wantCode:   ssoErrBadToken,
		},
		{
			...
			query: func() string {
				...
				return "token=" + spoke.MintSSOToken(spoke.SSOSigningSeedFromMaster(testHubSecret), "clubanderson", config.RoleOwner, testHiveID, time.Now().Add(wellPastAnyTTL))
			},
			wantStatus: http.StatusUnauthorized,
			wantCode:   ssoErrBadToken,
		},
