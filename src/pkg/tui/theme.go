...
// Theme is the TUI's palette: every color the frame draws, named by the ROLE
...
//
// WHY A STRUCT rather than loose vars. A token has exactly one definition and
// every call site names it, so the palette can be read in one place and a
// later change lands once. It is also the seam configurable themes (explicitly
// out of scope here) would plug into: swap the value of theme, not the call
// sites.
//
...
//
// WHERE THIS LIVES. Package tui, per the issue. Note the constraint that
// implies: pkg/tui imports pkg/tui/panes, so a PANE cannot reach these tokens
// without an import cycle. Today that costs nothing — panes/ renders no color
// at all, deliberately (see panes/tokens.go: the golden files pin output under
// termenv's Ascii profile, where color is stripped, so a colored pane would
// look one way in CI and another on a terminal). The first pane that genuinely
// needs a token should move this file to a leaf package both sides import,
// which is a rename and two import lines — not a reason to pre-emptively build
// the extra package now.
type Theme struct {
	// Border is the frame's de-emphasized chrome: the border around every
	// pane that does not have focus.
	Border lipgloss.AdaptiveColor

	// BorderFocus is the border around the focused pane. It is the frame's
	// only emphasis color, so it must read as clearly emphasized on either
	// background — see the contrast note on Theme.
	BorderFocus lipgloss.AdaptiveColor
}

// theme is the palette in force. Values are ANSI-256 indices, matching what
...
//     emphasis a light terminal that 205 carries on a dark one.
//
