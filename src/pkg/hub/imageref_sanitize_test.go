...
func TestSanitizeImageRefRoundTrip(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			in:   "registry.internal:5000/hivecommons/hive:v2-latest",
			want: "registry.internal:5000/hivecommons/hive:v2-latest",
...
func TestImageRefTagParseFailureIsSilent(t *testing.T) {
	cases := []struct {
		...
		{"registry port but no tag", "registry.internal:5000/hivecommons/hive", false, "port is not a tag"},
