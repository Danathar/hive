...
func TestSweepCompletedTaskListIssues(t *testing.T) {
	...
	fixtures := []taskListSweepFixture{
		...
		// #108 — all ticked + THIS Hive App bot author + NO trailer. Matching
		// bot authorship is a second positive hive-authorship signal, so this
		// MUST close.
		{
			Number:      108,
			Body:        "- [x] a\n- [x] b\n",
		...
		// #111 — all ticked + a different GitHub App bot + NO trailer. Bot
		// type alone is not proof this Hive filed the issue.
		{
			Number:      111,
			Body:        "- [x] a\n- [x] b\n",
		...
		// #112 — all boxes ticked, hive-filed, no merged PR references it.
		// #7071 gate: MUST NOT close.
		{
			Number:      112,
			Body:        "- [x] a\n- [x] b\n" + hiveTrailer,
			AuthorLogin: "hive-app[bot]",
...
func TestIsHiveFiledIssue(t *testing.T) {
	cases := []struct {
		...
		{"nil body + non-Bot is not hive-filed", "", "alice", "hive-app[bot]", "User", false},
		{"body with trailer + non-Bot is hive-filed", "some finding\n\n" + AttributionTrailerPrefix + " scanner", "alice", "hive-app[bot]", "User", true},
		{"body without trailer + non-Bot is not hive-filed", "some finding without any hive stamp", "alice", "hive-app[bot]", "User", false},
		{"matching Bot author with no trailer is hive-filed", "no trailer here", "hive-app[bot]", "hive-app[bot]", "Bot", true},
		{"matching Bot author (lowercase 'bot') is hive-filed", "no trailer here", "hive-app[bot]", "hive-app[bot]", "bot", true},
		{"different Bot author with no trailer is not hive-filed", "no trailer here", "other-app[bot]", "hive-app[bot]", "Bot", false},
		{"Bot author with unknown app login is not hive-filed", "no trailer here", "hive-app[bot]", "", "Bot", false},
		{"Bot author + trailer is hive-filed", "with trailer\n\n" + AttributionTrailerPrefix + " x", "other-app[bot]", "hive-app[bot]", "Bot", true},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			c := &Client{appBotLogin: tc.appBot}
			iss := &gh.Issue{
				Body: gh.Ptr(tc.body),
				User: &gh.User{Login: gh.Ptr(tc.login), Type: gh.Ptr(tc.authorType)},
			}
			if got := c.isHiveFiledIssue(iss); got != tc.want {
