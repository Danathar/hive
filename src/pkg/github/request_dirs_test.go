...
func TestRequestWatchersStayGatedInMain(t *testing.T) {
	...
	for _, calls := range [][]string{
		{"requestwatch.New(", "deps.startRequestRelays("},
		{"startRequestWatchers(", "deps.startRequestRelays("},
	} {
	...
	if strings.Contains(src, "deps.startRequestRelays(") {
		...
		for _, call := range []string{"github.PrepareRequestDirs", "requestwatch.New(", ".StartReviewRequestWatcher(", ".StartMergeRequestWatcher("} {
