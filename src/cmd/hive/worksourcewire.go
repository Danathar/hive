...
func (w *spokeWire) wireSpokeManagersAndLinear() {
	...
	if w.cfg.Governor.Rotation.Enabled {
		...
		if quotaPoolDir != "" {
			w.rotationMgr.EnableContributorReadingPublish(quotaPoolDir, quotaAccount)
			w.logger.Info("contributor quota reading publisher enabled", "pool_dir", quotaPoolDir, "explicit_pool_dir", explicitQuotaPoolDir)
		}
		...
		// kept off w.rotationMgr on purpose — rotation decision paths
		...
		w.quotaReadingPublisher = rotation.NewContributorReadingPublisher(quotaPoolDir, quotaAccount)
		w.quotaReadingPublisher.Start(w.ctx)
		w.logger.Info("contributor quota reading publisher enabled (publish-only; provider rotation disabled)",
