...
func TestBootDashboardAPIWith_RegistersDependenciesFromTheBootStruct(t *testing.T) {
	...
	if got, want := d.ResolveAppKeyFileFunc("", 99), appKeys.Resolve("", "/env/app.pem", 99); got != want {
		t.Fatalf("ResolveAppKeyFileFunc = %q, want %q (boot's appKeys.Resolve)", got, want)
	}
}
...
func TestBootDashboardAPIWith_ForgeInventoryResolvesTheActiveKey(t *testing.T) {
	...
	inv := f.forgeFn()
	if want := appKeys.Resolve(cfg.GitHub.KeyFile, os.Getenv("GH_APP_KEY_FILE"), cfg.GitHub.AppID); inv.ActiveKeyFile != want {
