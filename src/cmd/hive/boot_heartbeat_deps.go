...
type bootHeartbeatDeps struct {
	...
	startHeartbeat      func(ctx context.Context, hubURL string, collect spoke.StatusCollector, interval time.Duration, logger *slog.Logger, callbacks ...any)
	startTaskStatusPush func(ctx context.Context, hubURL string, collect spoke.TaskStatusCollector, logger *slog.Logger)
}
...
func defaultBootHeartbeatDeps() bootHeartbeatDeps {
	return bootHeartbeatDeps{
		publishIdentity: spoke.PublishHeartbeatIdentity,
		startHeartbeat: func(ctx context.Context, hubURL string, collect spoke.StatusCollector, interval time.Duration, logger *slog.Logger, callbacks ...any) {
			go spoke.StartHeartbeat(ctx, hubURL, collect, interval, logger, callbacks...)
		},
		startTaskStatusPush: func(ctx context.Context, hubURL string, collect spoke.TaskStatusCollector, logger *slog.Logger) {
			go spoke.StartTaskStatusPush(ctx, hubURL, collect, logger)
		},
	}
}
