...
func TestResolveCoverageBadgeURL(t *testing.T) {
	for _, org := range []string{"hivecommons", "kubestellar"} {
		if got := resolveCoverageBadgeURL("", org); got != "https://gist.githubusercontent.com/clubanderson/b9a9ae8469f1897a22d5a40629bc1e82/raw/coverage-badge.json" {
			t.Fatalf("default for %s: %q", org, got)
		}
	}
	// An adopter without its own badge shows 0 honestly rather than the
	// flagship project's number.
	if got := resolveCoverageBadgeURL("", "acme"); got != "" {
		t.Fatalf("adopter default: %q", got)
	}
	if got := resolveCoverageBadgeURL("https://x/badge.json", "acme"); got != "https://x/badge.json" {
