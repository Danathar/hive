...
| "agent restarts: NAME ×N/24h **(login token refreshed)**" | The token-triggered restart: the agent sits at a login prompt while a valid shared credential exists, so hive relaunches the CLI to nudge it into picking the credential up | Bounded at three consecutive attempts; after that hive stops and writes the reason into the agent's error line instead ("still at a login prompt after 3 restarts…"). Read that line — it names which credential file is the problem. **A count in the hundreds means the cap was not holding** ([#6578](https://github.com/hivecommons/hive/issues/6578), fixed): the give-up latch was being cleared by the boot pane of its own restart. These agents are blocked on a **human** device-flow login, which no number of restarts can supply — log in on the spoke dashboard |
