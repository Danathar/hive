...
The formal-verification quality-lane opt-in (`quality.formal`), long-running run checkpoint policy (`runs.checkpoints`, `runs.wait_timeout_seconds`, `runs.wait_severity`), and the persona learning opt-in (`persona.learning.enabled`, see [hosted-hub.md](hosted-hub.md#persona-learning-default-off)) also live in the dashboard under Settings -> Features.
...
hub:
  standby_contributors: [alice, bob]   # approval; volunteering is not approval
...
| a `standby` block | you expect this lane to pause for budget and you want the floor and the approved list written down before it does — the block is parsed and validated today and read by nothing |
...
- **[Documentation map](documentation-map.md)** — setup and the full topic-guide surface.
