...
read/write, authenticated with the dashboard token, and shaped as a `cmd/` client binary rather
than an endpoint — is designed separately in [admin-mcp.md](admin-mcp.md). Do not extend this
...
Remote contributor relays can opt in with `task_mcp.remote_enabled: true`. When a task is assigned, the hub mints an HMAC-signed lease bearer bound to the assignment tuple: task id, contributor session identity, repository, issue/PR number, the run stage present on the lease at mint time, and the same expiry as the hub's task lease. Unstaged leases leave the token stage empty. The lease token is stored by hash next to the task lease, along with the minted stage, so releasing, expiring, yanking, completing, or failing the task revokes MCP access with the same lifecycle as task ownership.
