...
| `HIVE_GIT_BOT_EMAIL_DOMAIN` | No | `hive.kubestellar.io` | Domain of the commit author/sign-off address `src/deploy/entrypoint.sh` writes into `/etc/gitconfig` when the derived git identity is a GitHub App bot (`<slug>[bot]`): the address becomes `<slug>@<domain>`. A plain hostname only (`[A-Za-z0-9.-]`); anything else falls back to the default. The bracketed `<slug>[bot]@users.noreply.github.com` GitHub itself uses is deliberately never written - probot-dco rejects it as a malformed email, so every commit authored with it fails DCO ([#6251](https://github.com/hivecommons/hive/issues/6251)). PR authorship and push stay with the App; only the commit author/sign-off address differs. |
