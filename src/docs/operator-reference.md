...
| `runs` | Long-running run settings; today only `runs.external.flue`, the report-only external-execution binding pilot. | Default OFF. `enabled: true` with no `mode` runs in `shadow` (persists admissions, observes, never starts external work); `mode: report-only` dispatches to a Flue runtime at `endpoint` pinned to `workflow_version`, with no repository credential, dashboard token, or publication tool. Requires a build with the `extwork_flue` tag. See [External workflow admission](design/external-workflow-admission.md). |
