...
| `ga4-anomaly-detector.sh` | Monitor | Compares recent GA4 event counts with a 7-day baseline and writes `/var/run/hive-metrics/ga4-anomalies.json`; emits a no-data result if GA4 is unavailable. Flags both error-count spikes and traffic collapses (the drop arm added for #6430) by delegating the computation to `ga4_anomaly_lib.py`. |
| `ga4_anomaly_lib.py` | Monitor | Pure, network-free anomaly computation for `ga4-anomaly-detector.sh`: the spike arm (recent error count exceeds a multiple of the 7-day baseline) and the drop arm (#6430 item 4 — an event/page collapsing below a floor of its baseline). Stdlib-only, split out so `test_ga4_anomaly_detector.sh` can unit-test it without a live GA4 property, a service-account key, or the google-analytics-data library. |
...
| `hive-review.sh` | Enforcement | Agent-side wrapper for `gh pr review`. Writes a review-request file the Hive submits with the App token and audits as `agent_pr_reviewed`, so PR-review activity is attributed and visible to hive-health (a direct `gh pr review` is invisible). Prints the matching `.result.json` path so callers can require confirmed delivery before dependent writes. `--comment --thread <id>` replies inside one review-bot thread and `--resolve-thread <id>` resolves it; the watcher refuses both on any thread a human opened (see `src/docs/review-bot-threads.md`). |
...
| `hive-podman-update.sh` | Deploy | Deliberate manual update and rollback for the Hive Quadlet unit (#4378). Updates are explicit rather than automatic, per ADR-0017. `reconcile` re-copies the repo-owned files a Quadlet host runs from — the gateway config and the unit files — which no update path refreshed before (#6078). |
...
| `omp-backend.js` | Contributor runtime | omp (Oh My Pi) contributor adapter (#7678). Container mode stages the host's omp sign-in — `~/.omp/agent/config.yml` plus the `agent.db` SQLite store whose `auth_credentials` rows hold every provider's OAuth record or API key; omp has no `auth.json` — into the ephemeral CLI staging dir and, like `pi-backend.js --stage`, keeps only the selected provider's rows (`AGENT_MODEL=provider/model`, else `config.yml`'s `modelRoles`). `--describe` is what `just contribute-setup omp` prints so a missing host sign-in is caught before a container opens on omp's setup wizard. `--stage` and `--describe` refuse/flag a row omp has marked `disabled_cause` (a revoked OAuth refresh), and `--sync-back` writes a credential the container refreshed back over the host row it was staged from — only the selected provider's existing row, only when strictly newer — so a single-use refresh token consumed in the container does not strand the host; `just contribute-hive omp` runs it every `HIVE_OMP_CREDENTIAL_SYNC_SECONDS` (default 300) while the container runs and once more at exit (#7922). |
...
| `test_backend_smoke_image_owner.sh` | backend-smoke's pinned lane must test THIS repository's contributor image, not upstream's (#6086): extracts the image-selection step from `backend-smoke.yml` and runs it, asserting an uppercase owner is lowercased and the upstream fallback is loud rather than silent. |
| `test_ga4_anomaly_detector.sh` | Contract test for `ga4_anomaly_lib.py`'s `compute_anomalies()` (#6430 item 4): hermetic Python assertions against synthetic recent/baseline dicts covering both the spike arm and the traffic-collapse drop arm — no GA4 property, key, or network. |
