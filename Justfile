...
contribute-hive backend="" mode="docker": check-version
    ...
    if [[ "$_MODE" == "local" ]]; then
      ...
      if [[ "$BACKEND" == "pi" ]]; then
        ...
        for name in ANTHROPIC_API_KEY OPENROUTER_API_KEY OPENAI_API_KEY OPENAI_HOST OPENAI_BASE_PATH GOOGLE_API_KEY GOOSE_API_KEY GOOSE_PROVIDER GOOSE_MODEL BOBSHELL_API_KEY HIVE_LITELLM_ENDPOINT HIVE_LITELLM_API_KEY KILO_AUTH_CONTENT KILO_CONFIG_CONTENT KILO_API_KEY KILO_ORG_ID META_API_KEY; do
...
contribute-hives *ARGS:
    #!/usr/bin/env bash
    set -euo pipefail
    # Usage: just contribute-hives                      # same as: hivectl hives list
    #        just contribute-hives use acme
    #        just contribute-hives add acme --hub wss://acme.example/contribute
    #
